...
// test
#define BHT_HIST_LENGTH 13
#define PHT_LOCAL_CTR_INIT 2
#define PHT_LOCAL_CTR_MAX  7
...
PREDICTOR::PREDICTOR(void){
  ...
  numPhtLocalEntries = (1<<(bht_history_length-1));
...
bool   PREDICTOR::GetPrediction(UINT32 PC){
  ...
  UINT32 pCC   = (PC^TOUR_LEN)%numTournamentCounter;
  UINT32 pCC1 = PC >> (32-TOUR_LEN);
  UINT32 pCC2 = (pCC * pCC1) % numTournamentCounter;
  //UINT32 pCC3 =
  if (predictorChooseCounter[pCC2] < 4) {
...
bool   PREDICTOR::GetLocalPrediction(UINT32 PC){
    UINT32 bhtIndex   = ((PC^bht_bit_size) % numBhtEntries);
...
void  PREDICTOR::UpdatePredictor(UINT32 PC, bool resolveDir, bool predDir, UINT32 branchTarget){
  ...
  if(resolveDir == TAKEN){
    ...
    pht_local[pht_local_index] = SatDecrement(SatDecrement(pht_local_counter));
  }
